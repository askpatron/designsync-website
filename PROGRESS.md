...
**Status:** live at **https://designs.trydesignsync.com** on `104.207.75.124` behind Cloudflare. Repo: **https://github.com/askpatron/designsync-website**. Owner deploys from the VPS only (`DEPLOYMENT.md`). Pushing `main` does not deploy; the GitHub workflow is validation-only and cannot rebuild leftover Docker. App companion handoff: `../designsync/PROGRESS.md`.
...
6. **Testimonials** — the template's unsupported quotes and “20+” claim were removed. The section sends clients to the public, no-login feedback form at `app.trydesignsync.com/feedback`. It loads the marketing carousel from `/feedback/published`, which returns only consented reviews an admin explicitly approved; private emails and pending/rejected submissions never enter the payload. Honest placeholders remain when there are no approved reviews or the API is unavailable. The planned hover-to-reveal video treatment remains a future enhancement.
